...
def extract_truth_positions(vcf_path: str) -> set:
    """Extract (chrom, pos, ref, alt) truth-set keys — allele-aware, so a
    position with multiple alleles is only matched against the specific
    allele actually called at that position (not any variant at that
    position)."""
    ...
    for rec in pysam.VariantFile(vcf_path):
        ...
        if rec.alts:
            for alt in rec.alts:
                truth_set.add((chrom, rec.pos, ref, str(alt)))
        else:
            truth_set.add((chrom, rec.pos, ref, "N"))
    logging.info(f"Extracted {len(truth_set)} truth (chrom, pos, ref, alt) keys")
    return truth_set


def label_variants(df: pd.DataFrame, truth_set: set, label_col: str = "TruthLabel") -> pd.DataFrame:
    """Label each variant as 1 if (chrom_norm, pos, ref, alt) is in
    truth_set, else 0 — vectorised. Allele-aware: matching on position alone
    would incorrectly label a different allele at the same site as a true
    positive (see label_variants_with_external_set, which already matched
    on the full allele)."""
    df = df.copy()
    ...
    df[label_col] = [1 if (c, p, r, a) in truth_set else 0
