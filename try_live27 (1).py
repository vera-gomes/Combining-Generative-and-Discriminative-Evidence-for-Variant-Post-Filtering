#!/usr/bin/env python3
"""
Variant Quality Scoring Pipeline - Refactored
Changes vs try_live17_lgb_hybrids_full_corrected.py:
  - Removed duplicate function definitions (kept the hybrid-aware versions)
  - Removed dead HybridAlphaSelector class
  - Integrated pairwise-comparison normalisation directly into PipelineConfig
    (removed module-level monkey-patch)
  - Fixed None > int crash in memory check when psutil is absent
  - Vectorised label_variants / label_variants_with_external_set
  - Fixed _parse_chrom_from_popdb_path to use regex
  - Fixed normalize_pairwise_comparisons (user pairs replace defaults; no forced merge)
  - Fixed p_one_sided / p_left redundancy in paired_bootstrap_delta
  - GM / BGM VQSLOD scores are now passed through sigmoid() before any
    threshold-based metrics, so their effective range is (0,1) and comparisons
    with probability models are fair.  score_model_instance now returns
    sigmoid-normalised scores for GM/BGM.
  - apply mode: config must supply paths.trained_model_dir pointing to the
    training output directory; a fresh timestamped output dir is still created
    for the apply run.
  - Moved gc import to top-level
"""
